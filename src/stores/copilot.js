...
function defaultWelcome() {
  return createMessage(
    "assistant",
    "Travel Copilot is online. Ask me for a tonight plan, budget split, safety cues, or offline sync guidance."
  );
}
...
function buildReply(promptText, scope, plannerContext, offlineStore) {
  ...
  if (query.includes("offline") || query.includes("sync") || query.includes("draft")) {
    ...
    return "Budget context is empty. Generate or open a trip first, then I will break down spending by category.";
  }
  ...
  if (query.includes("tonight") || query.includes("evening") || query.includes("today")) {
    if (plannerContext?.itineraryPreview?.length) {
      ...
      return `Tonight suggestion for ${destination}: ${tonight}. Keep a 20–30 minute commute buffer for smoother flow.`;
    }
    return "No active itinerary summary found for tonight planning. Generate a trip in Planner and try again.";
  }
