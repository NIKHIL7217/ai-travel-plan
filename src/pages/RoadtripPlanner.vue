...
async function handleGenerateRoadtrip() {
  ...
  if (!destination) {
    plannerError.value = "Destination is required for roadtrip intelligence.";
    return;
  }

  if (!isRoadtripMode(controls.value.travelMode)) {
    plannerError.value = "Choose a valid roadtrip mode: Car, Bike, or Bus.";
    return;
  }
  ...
  try {
    ...
    plannerError.value = error?.message || "Roadtrip intelligence could not be generated.";
    roadtrip.value = null;
  } finally {
    loading.value = false;
  }
